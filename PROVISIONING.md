# How to provision a new environment for Sun
...
* Enter webroot path: `/home/deployer/apps/Sun/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/Sun.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/Sun.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/Sun/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
9 9,21 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/Sun/shared/public/
```
